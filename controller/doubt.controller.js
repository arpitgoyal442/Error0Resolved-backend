...
const getAllDoubts = async (req, res) => {
  ...
  if (param.active == 1) filters.status = "active";
  else if (param.active == -1) filters.status = "inactive";

  if (param.topics) filters.topic = param.topics;

  if (param.sort == 1) sortOrder._id = -1;
  ...
  if (param.requested == 0 || param.requested == -1) {
    doubtSchema
      .find(filters, function (err, data) {
        if (err) res.status(404).send(err);
        else return res.send(data);
      })
      .sort(sortOrder);
  } else if (param.requested == 1) {
    let requested_doubts = await debuggerModel
      .findById(debuggerId)
      .then((data) => {
        console.log("data is");
        // console.log(data);
        ...
        doubtSchema
          .find(
            { _id: data.requestedDoubts, ...filters },
            function (err, data) {
              return res.send(data);
            }
          )
...
const editDoubt = async (req, res) => {
  ...
  doubtSchema
    .findByIdAndUpdate(doubtId, updateData)
    .then((data) => {
      console.log(data);
...
const addMessage = (req, res) => {
  let doubtId = req.params.doubtId;
  doubtSchema
    .findByIdAndUpdate(doubtId, { $push: { chats: req.body } })
    .then((data) => {
      return res.status(200).send(data);
    })
    .catch((err) => {
